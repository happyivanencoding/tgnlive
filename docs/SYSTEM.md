...
`eval/play.mjs` 已修复指定未知世界/天赋时静默回退到第一个世界的问题；真实负向检查在创建和模型调用前拒绝。manifest 区分工作区 HEAD、未提交改动和运行服务版本，taskId 不再硬编码旧任务。单次盲读比较 `eval/compare-growth.mjs` 不给分数或自动发布决定。基线/原著阅读收据与新批次结果见 `V060_RESEARCH_AND_EXPERIMENTS.md`。
