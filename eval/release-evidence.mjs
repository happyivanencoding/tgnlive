...
for(const label of labels){
 ...
 const ok=records.filter(r=>r.outcome==='completed'),traces=read(dir,'server-metrics.json',{turns:[]}).turns||[];
 ...
 const snapshot={label,requestedTurns:manifest.requestedTurns??summary.requestedTurns??18,committedTurns:ok.length,failedAttempts:records.filter(r=>r.outcome==='failed').length,fatal:summary.fatal??null,continuedFrom:manifest.continuedFrom||null,continuationReason:manifest.continuationReason||null,status:summary.endedAt?(summary.fatal?'stopped-with-error':'completed'):'in-progress',
